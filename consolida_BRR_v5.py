...
#______________________________________________________________________________________
#Função principal
def consolida_BRR(export, open_folder, abs_path, path_base, mensagem_text, output_format_var):
    ...
    for fname in lista_arqs:
        ...
        aux_df = importar_arquivo(path_ref)
    ...
    if flag == False:
        ...
        if flag_cont == 'S':
            ...
            if flag_cont2 == 'S':
                ...
                if export == True:
                    print('')
                    print(f'Exportando dados em formato .{output_format_var}...')
                    mensagem_text.insert(tk.END, '\n')
                    mensagem_text.insert(tk.END, f'Exportando dados em formato .{output_format_var}...\n')
                    mensagem_text.see('end')
                    mensagem_text.update()
                    fname = f"RESUMO_BRR_{rtp}RTP_{str(n_linhas)}_itens_{data_hj}.{output_format_var}"
                ...
                #Exporta a BRR em formato excel
                    ...
                    mensagem_text.see('end')
                    mensagem_text.update()
                    fname = f"BRR_{rtp}RTP_{str(n_linhas)}_itens_{data_hj}.{output_format_var}"
                    folder_path = '4_SAIDA_CONSOLIDA//'
                    path_exp = monta_path(abs_path, folder_path, fname)
                    exportar_arquivo(df_brr, path_exp, output_format_var)
                    print(f'    Arquivo exportado com sucesso!')
                    print(f'    {path_exp}')
                    mensagem_text.insert(tk.END, f'    Arquivo exportado com sucesso!\n')
                    mensagem_text.insert(tk.END, f'    {path_exp}\n')
...
def consolidar_brr(path_base_var, mensagem_text, output_format_var):
    ...
    export = True
    open_folder = True
    consolida_BRR(export, open_folder, abs_path, path_base, mensagem_text, output_format_var)
    #______________________________________EXECUÇÃO________________________________________
...
def make_frame(frame):
    ...
    label_formato.grid(row=1, column=0, padx=10, pady=10, sticky="w")
    ...
    dropdown_formatos.grid(row=1, column=1, sticky="w")

    # Botão Converter Base de Dados
    btn_converter_base_dados = tk.Button(frame, text="Consolidar BRR", bg="navy", fg="white", width=20, height=2, command=lambda: consolidar_brr(path_base_var, mensagem_text, output_format_var.get()))
    btn_converter_base_dados.grid(row=2, column=1, columnspan=3, pady=10, sticky="e")

    # Display
    mensagem_label = tk.Label(frame, text="Mensagens:", fg="blue")
    mensagem_label.grid(row=6, column=0, pady=(90, 0), padx=10, sticky="w")
